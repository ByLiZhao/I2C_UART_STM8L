  ...
  typedef struct {
    ...
    union {/* bytewise access to register */
      ...
      struct {/* bitwise access to register */
        ...
        uint8_t NOSTRETCH : 1;    /* Clock stretching disable (Slave mode). 0: enabled, 1: disabled. */
      } reg;
    } CR1;
    ...
    union {/* bytewise access to register */
      uint8_t  byte;
      struct {/* bitwise access to register */
        uint8_t START     : 1;    /* Start generation. master 0: start gen. 1: continous start gen. */
																	/* slave 0: no start generation, 1: start gen when bus is free */
        uint8_t STOP      : 1;    /* Stop generation. 0: no stop generation. master 1: stop gen. salve 1 :release lines */
        uint8_t ACK       : 1;    /* Acknowledge enable. 0: no ack, 1: ack returned when byte received. */
        uint8_t POS       : 1;    /* Acknowledge position (for data reception). 0: current byte, 1: next byte */
				uint8_t PEC				: 1;		/* packet error checking. 0: no PEC transfer, 1: PEC transfer. */
				uint8_t ALERT			: 1;		/* SMBus alert. 0: Releases SMBAlert pin high. 1: Drives SMBAlert pin low */
    ...
    union {/* bytewise access to register */
      uint8_t  byte;
      struct {/* bitwise access to register */
        uint8_t ADD0      : 1;    /* Interface address [0] (in 10-bit address mode only) 7-bit addressing mode don't care */
    ...
    union {/* bytewise access to register */
      ...
      struct {/* bitwise access to register */
        uint8_t res       : 1;    /* Reserved */
        uint8_t ADD       : 2;    /* Interface address [9:8] (in 10-bit address mode only) */
        uint8_t res2      : 3;    /* Reserved */
        uint8_t ADDCONF   : 1;    /* Address mode configuration, must always be 1 */
        uint8_t ADDMODE   : 1;    /* 7-/10-bit addressing mode (Slave mode) 0: 7bit slave 1: 10 bit slave */
      } reg;
    } OARH;
		...
		union {
			uint8_t byte;
			struct {
				uint8_t ENDUAL			: 1;	/* Dual addressing mode enable 0: use only UAR1, 1: use both OAR1 and OAR2. */
    ...
    union {/* bytewise access to register */
      uint8_t  byte;
      struct {/* bitwise access to register */
        uint8_t SB        : 1;    /* Start bit (Mastermode) 0: no start condition, 1: start condition generated */
        ...
        uint8_t ADDR      : 1;    /* Address sent (master mode) / matched (slave mode) */
																	/* master 0: no end of address transmission. 1: end of address transmission */
																	/* slave 0: address mismatched or not received. 1: received address matched. */
        uint8_t BTF       : 1;    /* Byte transfer finished. 0: not done, 1: finished. */
        uint8_t ADD10     : 1;    /* 10-bit header sent (Master mode) */
        uint8_t STOPF     : 1;    /* Stop detection (Slave mode) 0: no stop condition detected. 1: stop condition detected */
        uint8_t res       : 1;    /* Reserved */
        uint8_t RXNE      : 1;    /* Data register not empty (receivers) 0: data register empty, 1: data register not empty */
        uint8_t TXE       : 1;    /* Data register empty (transmitters) 0: data register NOT empty, 1: empty */
      } reg;
    } SR1;
    ...
    union {/* bytewise access to register */
      ...
      struct {/* bitwise access to register */
        ...
        uint8_t ARLO      : 1;    /* Arbitration lost (master mode) 0: no arbitration lost 1: arbitration lost detected */
    ...
    union {/* bytewise access to register */
      uint8_t  byte;
      struct {/* bitwise access to register */
        uint8_t ITERREN   : 1;    /* Error interrupt enable 0: disable. 1: enable. rw */
        ...
        uint8_t ITBUFEN   : 1;    /* Buffer interrupt enable */
				...
				uint8_t LAST			: 1;		/* DMA last transfer 0: Next DMA EOT is not the last transfer */
    ...
    union {/// bytewise access to register */
      ...
      struct {/// bitwise access to register */
        ...
        uint8_t res       : 2;    /* Reserved */
        uint8_t DUTY      : 1;    /* Fast mode duty cycle. 0 t_low/t_high =2. 1: t_low/t_high = 16/9 */
        uint8_t FS        : 1;    /* I2C master mode selection 0: standard mode. 1: fast mode. */
      } reg;
    } CCRH;
