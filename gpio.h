...
#define pin_output_reg(pPort,pin)   ((pPort)->ODR.bit.pin)  ///< pin output register (1 pin)
#define pin_input_reg(pPort,pin)    ((pPort)->IDR.bit.pin)  ///< pin input register (1 pin)

// direct port read/write via bytewise access (port)
#define port_output_reg(pPort)      ((pPort)->ODR.byte)     ///< port output register (8 pins)
#define port_input_reg(pPort)       ((pPort)->IDR.byte)     ///< port input register (8 pins)

#define pin_output_high(pPort,pin)        ((pPort)->ODR.byte  |= (0x01<<pin))      ///< set pin output state high
#define pin_output_low(pPort,pin)         ((pPort)->ODR.byte  &= ~(0x01<<pin))     ///< set pin output state high
#define pin_output_toggle(pPort,pin)      ((pPort)->ODR.byte  ^= (0x01<<pin))      ///< toggle pin output state
#define pin_output_set(pPort,pin,state)   (state ? pin_output_high(pPort,pin) : pin_output_low(pPort,pin)) ///< set pin output state
#define pin_input_read(pPort,pin)        (((pPort)->IDR.byte & (0x01<<pin))>>pin) ///< read pin input state
