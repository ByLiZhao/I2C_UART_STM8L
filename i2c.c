...
/**
  ...
  configure I2C bus as master in standard mode, BR=100kHz, 7bit address, 250ns
  rise time
*/
...
/* set clock */
#define CLK_CKDIVR_HSIDIV                                                      \
  ((uint8_t)0x18)                    /*!< High speed internal clock prescaler */
...
#define CLOCK_I2C ((uint8_t)0x08)    /*bit mask to enable I2C clock */
  ...
  /*reset i2c bus*/
  I2C.CR1.byte = I2C_CR1_RESET_VALUE; /* Peripheral disabled, general call
                                         diaabled, no clock stretching */
  ...
  I2C.TRISER.reg.TRISE = 0x11; /* 17, t_master = 1000/16=62.5ns. (1000/62.5)+1.
                                  1000ns is maximum allowed rising time */
  I2C.CR1.reg.ENGC = 0;        /* general call disabled. */
  I2C.CR1.reg.NOSTRETCH =
      1; /* clock strench disabled, sensor does not use strench */
  for (i = 0; i < 4; ++i) {
    count = 10000;
    while (--count)
      ;
...
/**
  \fn uint8_t i2c_write(uint8_t addr, uint8_t offset, uint8_t numTx, uint8_t
  *bufTx) \ only support 7-bit address mode \brief write data via I2C \param[in]
  addr        7b address [6:0] of I2C slave
        \param[in]  offset			offest or register address of
  the slave device. \param[in]  numTx       number of bytes to send \param[in]
  bufTx       send buffer \return error code (0=ok; 1=timeout) write data to I2C
  slave with frame timeout. Note that no start or stop condition is generated.
*/
uint8_t i2c_write(uint8_t addr, uint8_t offset, uint8_t numTx, uint8_t *bufTx) {
  uint8_t i = 0;
  uint16_t countTimeout =
      10000; /* use counter for timeout to minimize dependencies */
  uint8_t temp = 0;
  ...
  while ((!I2C.SR1.reg.ADDR) && (--countTimeout))
    ; /* wait until address sent or timeout */
  if (countTimeout == 0)
    return (3);
...
/**
  \fn uint8_t i2c_read(uint8_t slaveAddr, uint8_t offest, uint8_t numRx, uint8_t
  *bufRx)

  \brief request data via I2C as master
  \param[in]  slaveAddr   7b address [6:0] of I2C slave
        \param[in]  offset			offset or register address of
  slave device. \param[in]  numRx       number of bytes to receive \param[out]
  bufRx       receive buffer \return error code (0=ok; 1=timeout) request data
  from I2C slave with frame timeout. Note that no start or stop condition is
  generated.
*/
uint8_t i2c_read(uint8_t addr, uint8_t offset, uint8_t numRx, uint8_t *bufRx) {
  ...
  /* send 7b slave adress [7:1] + write flag (LSB=0) */
  I2C.DR.byte = (uint8_t)((addr << 1) &
                          ~0x01); /* shift left and set LSB (write=0, read=1) */
  countTimeout = 10000;           /* ~1.1us/inc -> ~10ms */
  while (!I2C.SR1.reg.ADDR && --countTimeout)
    ; /* wait until address sent or timeout */
  if (!I2C.SR1.reg.ADDR)
  ...
  if (numRx == 1) {
    ...
    I2C.CR2.reg.POS =
        1; /*set pos, next byte received in shift register will be NACKed */
    ...
    while ((!I2C.SR1.reg.BTF) && --countTimeout)
      ; /*because clock strench enabled.
...
*/
    ...
    countTimeout = 1000; /* ~1.1us/inc -> ~1ms */
    while ((!I2C.SR1.reg.BTF) && --countTimeout)
      ; /*pull BTF, BTF==0 means byte transfer not succeed */
    if (!I2C.SR1.reg.BTF)
      return (12);
    ...
    while (num_to_read > 3) {
      countTimeout = 1000; /* ~1.1us/inc -> ~1ms */
      while (!I2C.SR1.reg.BTF &&
             --countTimeout) { /* pull BTF, BTF==0 means byte transfer not
                                  succeed */
        if (countTimeout-- == 0)
          return (14);
      } /*while BTF*/
