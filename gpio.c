...
void port_mode(PORT_t * port_ptr, uint8_t dir, uint8_t cr1, uint8_t cr2) {
   ...
   port_ptr->CR1.byte = cr1;                       /* input: 0=float, 1=pull-up; output: 0=open-drain, 1=push-pull */ \
   port_ptr->CR2.byte = cr2;                       /* input: 0=no exint, 1=exint; output: 0=2MHz slope, 1=10MHz slope */ \
